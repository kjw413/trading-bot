...
사실이지만 배치 로그만 봐서는 알 수 없는 세 가지입니다.
...
- 재무(fundamentals) 수집은 종목↔DART corp_code 매핑이 아직 없어, **유효한
  `DART_API_KEY`를 설정해도 당분간 `성공 (0행)`으로 보고됩니다.** 실제 수집은
  Phase 3에서 매핑이 추가된 뒤 시작됩니다.
