...
| 데이터 | 소스 | 필요한 것 |
|---|---|---|
| 가격(OHLCV), 시장·거시 지표 | FinanceDataReader | 없음 |
| 투자자 수급, 밸류에이션(PER/PBR 등) | pykrx → KRX | [data.krx.co.kr](https://data.krx.co.kr) 무료 회원가입 후 환경변수 `KRX_ID`, `KRX_PW` |
| 재무제표 | DART OpenAPI | [opendart.fss.or.kr](https://opendart.fss.or.kr) 무료 API 키를 환경변수 `DART_API_KEY`로 설정 |
...
로그가 초록색이라고 해서 운영자가 기대하는 모든 것이 이뤄진 건 아닙니다. 지금 시점에
사실이지만 배치 로그만 봐서는 알 수 없는 두 가지입니다.

- **`data pipeline`은 가격(OHLCV) 캐시를 갱신하지 않습니다.** 가격은 기존 `data update`
  명령이 담당하며, 두 가지를 모두 실행해야 합니다. (Phase 3에서 통합 예정)
- 품질 검사 모듈(`data/quality.py`)은 구현되어 있으나 **아직 일일 배치에 연결되어
  있지 않습니다.** 따라서 지금은 손상된 값·중복 키·가용일 역전이 배치에서 자동으로
  걸러지지 않습니다. (Phase 3에서 수집 직후 검사 단계로 편입 예정)

재무(fundamentals) 수집은 `DART_API_KEY`만 설정하면 동작합니다. 종목코드↔corp_code
매핑(`data/corp_codes.py`)이 DART 전체 기업목록을 받아 `data/cache/_listings/`에
30일간 캐시하며, 갱신에 실패하면 오래된 캐시로 계속 진행합니다 — 몇 주 지난 매핑도
거의 전부 맞고, 일시적 장애로 수집을 멈추는 게 더 손해이기 때문입니다.
