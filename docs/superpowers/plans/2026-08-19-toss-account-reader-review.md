...
## 8. 재검토 결과 — 통과 (커밋 `a3258c5`)

세 결함 모두 §1~§3에 적은 대로 수정되었고, **같은 재현 절차로 다시 확인했다.**

| | 수정 전 | 수정 후 |
|---|---|---|
| 결함 1 원화 전용 계좌 | `value_krw()` → `KeyError: 'USD'` | `12,200,000` (저장→재로드 후에도 동일) |
| 결함 1 빈 계좌 | `KeyError: 'USD'` | `5,000,000`, `weights={}` |
| 결함 2 `amount.usd = "0"` | `TossError` 로 실행 중단 | 정상, 교차 검증만 건너뜀 |
| 결함 3 폴백 + 403 | `fx_source='macro'` 로 조용히 통과 | `TossIPNotAllowedError` |

추가로 직접 확인한 것:

- 돈이 하나도 없는 새 계좌(보유 0, 현금 0)도 `value_krw() == 0`, `weights == {}`,
  렌더까지 정상 — 0으로 나누는 자리가 없다.
- 원화 전용 계좌를 두 번 실행하는 경로가 성립한다: `days=14`, `+5.9%`,
  주가 몫 = 전체, 환율 몫 = 0.
- `state/account/`에 저장한 뒤 다시 읽어도 평가액이 같다.

§4의 회귀 테스트 7개가 모두 들어왔고, 기존 두 테스트에는 `value_krw()` 호출이
덧붙었다. **878개 통과** (871 + 7), 회귀 0. `broker/` 파일 0개.

§5도 처리되었다: 안내 문구가 "의존성 설치" 뜻으로 바뀌었고, 도달 불가 코드가 빠졌고,
독스트링/주석이 1줄 → 12줄로 늘어 §5.2에서 지목한 세 근거(`midRate`를 쓰는 이유,
현금 조회 실패가 예외인 이유, transport 가 상태 코드를 돌려주는 이유)가 코드에 들어왔다.
최장 줄이 166자 → 100자로 줄었다.

**M1은 이제 사용자의 실계좌 대조(설계서 §12)만 남았다.** 다음 설계는
`2026-08-19-weekly-briefing-m2-news-design.md`이며, 그 §0이 어느 태스크가 이 대조를
기다려야 하는지 적어 두었다.
