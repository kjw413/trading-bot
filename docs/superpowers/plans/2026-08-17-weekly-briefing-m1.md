...
- 파일 쓰기는 `encoding="utf-8"` 명시. `.bat`도 UTF-8이며, 추가 조건은 아래 참조.
- ~~**`.bat` 파일은 ANSI/CP949로 저장한다.**~~ **← 2026-08-20 정정: 이 규칙이 틀렸다.**
  당시 `트레이딩봇 실행.bat` 상단 주석("cmd는 OEM 코드페이지로 파싱한다")을 근거로
  삼았는데, 그 주석은 구형 conhost 시절의 설명이다. Windows 11은 `.bat`을 Windows
  Terminal에서 열고 그 콘솔은 레지스트리 OEMCP가 949여도 **코드페이지 65001(UTF-8)**
  이다. 그래서 CP949로 저장한 `주간 브리핑.bat`은 모든 `echo` 뒤가 깨져 나왔다.
  (그 주석은 2026-08-21에 세 런처를 UTF-8로 통일하면서 제거했다.)
  **올바른 규칙:** `.bat`은 **UTF-8(BOM 없이)** 로 저장하고 첫 한글 줄보다 앞에
  `chcp 65001 > nul`을 둔다 (`데이터 수집.bat`이 이미 그렇게 되어 있다).
  그 줄 **위쪽의 명령**은 ASCII여야 한다. REM 주석의 한글은 무해하다 — UTF-8 한글은
  모든 바이트가 0x80 이상이라 그 안에 ASCII 특수문자가 숨을 수 없고, 콘솔이 아직
  CP949여도 파서가 어긋나지 않는다.
...
1. **`주간 브리핑.bat`** — `트레이딩봇 실행.bat`을 본떠 만들되:
   - ~~**ANSI/CP949로 저장한다.**~~ **2026-08-20 정정:** UTF-8(BOM 없이) + 첫 한글 줄
     앞에 `chcp 65001 > nul`. 이유는 위 「지켜야 할 것」의 정정 항목에 적었다.
