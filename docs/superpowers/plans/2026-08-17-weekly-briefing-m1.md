...
- ~~**`.bat` 파일은 ANSI/CP949로 저장한다.**~~ **← 2026-08-20 정정: 이 규칙이 틀렸다.**
  `트레이딩봇 실행.bat` 상단 주석("cmd는 OEM 코드페이지로 파싱한다")을 근거로 삼았는데,
  그 주석은 구형 conhost 시절의 설명이다. Windows 11은 `.bat`을 Windows Terminal에서
  열고 그 콘솔은 레지스트리 OEMCP가 949여도 **코드페이지 65001(UTF-8)** 이다. 그래서
  CP949로 저장한 `주간 브리핑.bat`은 모든 `echo` 뒤가 깨져 나왔다.
  **올바른 규칙:** `.bat`은 **UTF-8(BOM 없이)** 로 저장하고 첫 한글 줄보다 앞에
  `chcp 65001 > nul`을 둔다 (`데이터 수집.bat`이 이미 그렇게 되어 있다).
  그 줄 위쪽은 전부 ASCII여야 파서가 어긋나지 않는다.
