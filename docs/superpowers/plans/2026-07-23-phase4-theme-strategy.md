...
- ~~**데이터 신선도 게이트는 "전 종목 NaN"만 검사하고 "가격 캐시가 오래됨"은 검사하지 않는다.**~~ **(2026-07-26 해소)** `_is_price_data_stale`이 유니버스에서 가장 최근 봉과 `dt` 사이의 거래일 간격을 재고, `max_staleness_days`(기본 3거래일)를 넘으면 경고와 함께 리밸런싱을 건너뛴다. 신선도는 유니버스 전체 기준이라 거래정지 종목 하나가 전체를 막지 않는다.
