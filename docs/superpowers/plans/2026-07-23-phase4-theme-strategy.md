...
- **데이터 신선도 게이트는 "전 종목 NaN"만 검사하고 "가격 캐시가 오래됨"은 검사하지 않는다.** `data update` 작업이 조용히 멈춰도 캐시된 가격이 있으면 계속 리밸런싱한다. 모의투자 장기 운영(M15) 전에 마지막 가격 날짜와 거래소 캘린더의 직전 거래일을 비교하는 신선도 검사를 추가해야 한다.
