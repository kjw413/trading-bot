...
### 6. **잠정실적과 정기보고서를 섞어 읽으면 추정이 절반으로 깎인다** (실데이터가 잡은 버그)

Step 6을 실행한 결과 `events`가 8종목 11년에 **884행** 나왔다. 종목당 연 10건
— 분기 실적이면 4건이어야 한다. 대형주는 잠정실적과 정기보고서를 **둘 다**
내기 때문이다.

패널이 둘 다 담는 것은 설계대로다(잠정실적을 안 내는 중소형주는 정기보고서가
대체 이벤트). 틀린 것은 **소비하는 쪽**이었다. `_event_schedule`이 패널의
날짜를 전부 읽어 간격 중앙값을 냈다:

```text
잠정 1/9 → 분기보고서 3/12 (63일) → 잠정 4/5 (24일) → 반기보고서 5/16 (41일)
```

중앙값이 91일이 아니라 약 40일이 된다. **6주마다 발표가 있다고 추정하므로 그
종목은 사실상 상시 이벤트 창 안에 있게 된다.**

수정: `data/events.py`에 `schedule_dates()`를 추가해 **한 종류만** 반환한다.
잠정실적이 추정 가능한 수(4건) 이상이면 그것만, 아니면 정기보고서만. 절대 섞지
않는다.

이 버그도 순수 함수 단위 테스트로는 드러나지 않았다 — 테스트 fixture가 잠정실적만
담고 있었기 때문이다. **테스트 데이터가 실제 데이터의 모양을 갖고 있지 않으면
통과는 아무것도 보증하지 않는다.** E2E 테스트에 정기보고서를 추가해도 결과가
변하지 않는지 확인하는 케이스를 넣었고, 수정을 되돌리면 실패하는 것을 확인했다.

### 7. **`provisional` 안에도 분기당 2건이 있었다** (실데이터 2차 진단)

6번을 고치고 실제 패널을 들여다보니, 종류를 안 섞어도 여전히 깨져 있었다.
삼성전자 2024년 상반기 실측:

| 날짜 | 보고서명 | 실제 정체 |
|---|---|---|
| 01-09 | 연결재무제표기준영업(잠정)실적(공정공시) | **Q4 이벤트** |
| 01-31 | 매출액또는손익구조30%(대규모법인은15%)이상변경 | 같은 실적의 다른 공시 |
| 03-12 | 사업보고서 (2023.12) | periodic |
| 04-05 | 연결재무제표기준영업(잠정)실적(공정공시) | **Q1 이벤트** |
| 04-30 | **[기재정정]**연결재무제표기준영업(잠정)실적(공정공시) | 04-05의 정정 |
| 05-16 | 분기보고서 (2024.03) | periodic |

`provisional` 간격이 22, 65, 25일 → 중앙값 ~25일. 분기가 아니다.

원인 두 가지. **정정공시(`[기재정정]`)** 를 원공시와 같은 종류로 분류했고 —
계획의 테스트가 "정정된 잠정실적도 잠정실적이다"라고 **명시적으로 틀린 주장을
단언**하고 있었다 — **매출액또는손익구조** 공시를 잠정실적 마커에 함께 넣었다.
후자는 임계값을 넘을 때만 나오므로 애초에 규칙적인 계열이 아니다.

수정: `event_kind`를 4종으로 분리했다.

```text
provisional       분기 잠정실적 — 유일한 진짜 이벤트이자 유일한 규칙적 계열
amendment         [기재정정] — 시장은 원공시에 반응했다. 새 이벤트가 아니다
structure_change  매출액또는손익구조 — 실적 정보이나 비정기
periodic          분기/반기/사업보고서 — 발표보다 수 주 늦다
```

`_SCHEDULE_KINDS = ("provisional", "periodic")`만 일정 추정에 쓴다. 나머지 둘은
패널에 남긴다 — M5 이벤트 스터디가 쓸 수 있는 실적 정보이기 때문이다.

수정 후 삼성전자 간격: **91, 91, 96, 90, 87일.** 2024-07-01 기준 다음 발표까지
4일로 추정하며, 실제 Q2 잠정실적은 7월 초에 나왔다.

`EVENTS_DATA_VERSION`을 2로 올렸다. **버전 1로 수집된 행은 낡은 분류를 갖고
있으므로 데이터셋을 지우고 다시 수집해야 한다.**

교훈: 계획의 테스트가 실제 보고서명 세 개 중 하나(`매출액또는손익구조...이상변동`)를
**기억으로 지어냈고 실제 문자열은 `이상변경`이었다.** 지어낸 fixture는 분류기가
틀린 채로도 통과한다. 이제 테스트의 보고서명은 전부 실측값이다.
