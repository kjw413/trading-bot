...
- **환율을 스냅샷 안에 박아 넣는다.** 나중에 다시 계산해도 같은 숫자가 나와야 한다.
  응답에 환율이 없으면 `data/macro.py:32`의 `usdkrw`로 채우고, 어느 쪽에서 왔는지
  스냅샷에 기록한다.
...
**입출금을 어떻게 아는가 — 이것이 M1의 가장 큰 미결이다.** 토스 API에 입출금 내역
조회가 있으면 그대로 쓴다(§10-1에서 확인). 없다면 스냅샷 두 개만으로는 입금과
수익을 구분할 수 없다. 그 경우의 처리를 미리 정한다.
...
가진 VPS**로 좁혀진다. GitHub Actions는 러너 IP가 매번 바뀌어 쓸 수 없다.
...
이 검토 환경에서 `developers.tossinvest.com` 접속이 프록시로 차단되어 공식 스펙을
직접 읽지 못했다. 아래는 **코드를 쓰기 전에** 확인해야 한다.

1. **입출금 내역 조회 API가 있는가.** §4.2의 설계가 여기서 갈린다. 있으면 TWR이
   정확해지고, 없으면 "측정 불가" 경로가 상시 동작한다.
2. **읽기 전용 스코프가 있는가.** §6의 전제.
3. **해외주식 잔고 응답에 원화 평가액과 적용 환율이 오는가.** 오면 §4.1의 환율
   처리가 크게 줄어든다.
4. **소수점 보유 수량이 응답에 어떻게 오는가.** `Holding.qty`를 float로 잡은
   이유이지만, 실제 표현을 확인해야 한다.
5. 정확한 토큰 유효기간 — 자료마다 1시간과 24시간이 엇갈린다. 캐시 만료 로직이
   이 값에 의존한다.
6. 허용 IP를 몇 개까지 등록할 수 있는가 — 개발용 PC와 VPS를 동시에 등록할 수
   있으면 개발이 훨씬 편해진다.
